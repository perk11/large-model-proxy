...
func TestValidConfigMinimal(t *testing.T) {
	...
	config, err := loadConfigFromString(t, `{
		"ResourcesAvailable": {
            ...
            "VRAM-GPU-2": {"Amount": 1000, "CheckCommand": "Look, I am an object!", "CheckWhenNotEnoughIntervalMilliseconds": 4000},
		},
	...
	assert.Equal(t, uint(4000), config.ResourcesAvailable["VRAM-GPU-2"].CheckWhenNotEnoughIntervalMilliseconds)
...
func TestJsonWithCheckCommand(t *testing.T) {
	...
	assert.Equal(t, uint(1000), config.ResourcesAvailable["RAM"].CheckWhenNotEnoughIntervalMilliseconds)
}
func TestJsonWithNonIntegerInCheckInterval(t *testing.T) {
	t.Parallel()
	_, err := loadConfigFromString(t, `{
		"ResourcesAvailable": { "CheckCommand": "Bar", "CheckWhenNotEnoughIntervalMilliseconds": "string" },
	}`)
	checkExpectedErrorMessages(t, err, []string{
		"each entry in ResourcesAvailable must be an integer or an object with at least one of the fields",
	})
