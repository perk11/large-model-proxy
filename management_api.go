...
func handleStatus(responseWriter http.ResponseWriter, request *http.Request, services []ServiceConfig) {
	...
	// Snapshot connection stats under connectionStatsMutex ONCE, BEFORE taking
	// serviceMutex. This keeps the documented lock invariant: the two
	// mutexes are never held at the same time. Status is a point-in-time,
	// best-effort snapshot, so reading the counts a moment earlier than the
	// runningServices data is acceptable.
	resourceManager.connectionStatsMutex.Lock()
	connectionStatsSnapshot := make(map[string]ServiceConnectionStats, len(services))
	for _, service := range services {
		connectionStatsSnapshot[service.Name] = resourceManager.connectionStats[service.Name]
	}
	...
	for resourceName, resourceConfig := range config.ResourcesAvailable {
		...
		if resourceConfig.CheckCommand == "" {
			...
			// resourcesAvailable is written by the resource monitor goroutine
			// under resourcesAvailableMutex, so the read must take the same lock.
		...
		response.Resources[resourceName] = ResourceUsage{
			// Total is the statically configured Amount. For resources backed solely by
			// a CheckCommand, Amount is typically 0, so Total is omitted (via omitempty)
			// and the dashboard renders "N/A" — the capacity is only known dynamically
			// from the check command, which is reported via Free instead.
	...
	for _, service := range services {
		...
		connectionStats := connectionStatsSnapshot[service.Name]
